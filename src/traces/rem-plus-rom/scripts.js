/**
 * Rem + Rom - Animated circle visualization
 */
...
/**
 * Point class for circle vertices
 */
...
/**
 * Circle class for animated blob shapes
 */
var Circle = function(count, normalLength, radiusIn, radiusOut, color, fill, glow) {
  ...
  this.fill = fill;
  this.glow = glow;
  this.angleSpeed = (Math.random() - 0.5) * RAD * 0.1;
  this.points = [];
  this.init();
};
...
Circle.prototype = {
  ...
  update: function() {
    ...
    this.points.forEach(function(p) {
      ...
      p.radius = scope.radiusIn + (0.5 + (0.5 * Math.cos(p.radiusAngle))) * scope.radiusOut;
...
/**
 * Initialize circles with given parameters
 */
