/**
 * Geometry utilities for polygon intersection and rendering
 */
  ...
  intersectPolygon: function(a_p1, a_r1, polygon, mode) {
    ...
    for (var i = 0; i < points.length; i++) {
      ...
      if (Math.abs(colSet.tValue) > 1e-5) {
        if (mode === 1 && colSet.tValue > 0) {
          collisionCandidates.push(colSet);
        }
        if (mode === 2 && colSet.uValue > 0) {
          collisionCandidates.push(colSet);
        }
        if (mode === 0) {
...
RegularPolygon.prototype = {
  ...
  drawRounded: function(radius, g, offset) {
    ...
    for (var i = 0; i < this.points.length; i++) {
      ...
      var rL = pL.subtract(pI).normalize();
      var rN = pN.subtract(pI).normalize();
    ...
    for (var i = 0; i < pts.length; i++) {
      ...
      var radInset = (radiusI / Math.sin(thisAngle));
...
function update() {
  ...
  for (var y = 0; y < 43; y++) {
    for (var i = 0; i < count; i++) {
      ...
      var rpI = new RegularPolygon(
        new Point(50 + 17 * i, 30 * y + (even ? 200 : 190) - 200),
        20.2,
        3,
        even ? 0 : Math.PI
      );
